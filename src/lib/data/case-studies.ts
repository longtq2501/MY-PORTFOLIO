...
export const caseStudies: CaseStudy[] = [
    ...
    // ─────────────────────────────────────────────
    {
        ...
        accentColor: "#7c6aff",
        stack: [
            "Next.js 15", "Spring Boot 3.4 (Java 21)" , "React 19", "TypeScript 5", "Tailwind CSS 4", "Shadcn/UI",
        ...
        overview: {
            ...
            vi: "Hầu hết gia sư phải xoay xở với bảng tính, ứng dụng nhắn tin và chuyển khoản ngân hàng thủ công để vận hành lớp học. Tutor Pro thay thế tất cả bằng một nền tảng thống nhất: lập lịch tự động, thông báo thời gian thực, phòng học trực tuyến WebRTC, phản hồi do AI tạo và xuất hóa đơn chuẩn tài chính — đã triển khai và đang vận hành thực tế.",
        },
        sections: [
            {
                type: "features_list",
                ...
                features: [
                    {
                        ...
                        problem: { en: "Initializing an entire month of sessions for 30+ students one-by-one is unusable in practice.", vi: "Khởi tạo từng buổi học cho hơn 30 học sinh trong cả tháng một cách thủ công là điều không khả thi trong thực tế." },
                        solution: { en: "Optimistic Batch Processing with In-Memory Deduplication (O(1) conflict detection), Single-Pass Database Query, and JDBC Batch Inserts for transactional integrity.", vi: "Xử lý hàng loạt (Batch Processing) với cơ chế khử trùng lặp trong bộ nhớ (O(1)), Truy vấn cơ sở dữ liệu một lần và JDBC Batch Insert để đảm bảo tính toàn vẹn giao dịch." },
                        result: { en: "300+ sessions generated across a full student roster in < 800ms.", vi: "Tạo hơn 300 buổi học cho toàn bộ danh sách học sinh trong chưa đầy 800ms." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Students need to know instantly when a session is rescheduled, a grade is posted, or an exam is assigned — without polling.", vi: "Học sinh cần biết ngay lập tức khi buổi học bị dời lịch, có điểm mới hoặc bài kiểm tra được giao — mà không cần tải lại trang." },
                        solution: { en: "Server-Sent Events over Spring Application Events. SseEmittersManager maintains a thread-safe ConcurrentHashMap of emitters per user, with a heartbeat protocol to survive Docker/proxy connection drops and auto-reconnect on the client.", vi: "Sử dụng Server-Sent Events (SSE) dựa trên Spring Application Events. SseEmittersManager duy trì một ConcurrentHashMap thread-safe cho mỗi người dùng, cùng với giao thức heartbeat để duy trì kết nối qua Docker/proxy và tự động kết nối lại ở phía client." },
                        result: { en: "End-to-end delivery in < 500ms. idx_recipient_read index keeps unread-count queries at < 10ms regardless of volume.", vi: "Giao nhận thông báo dưới 500ms. Chỉ mục idx_recipient_read giúp truy vấn số lượng chưa đọc dưới 10ms bất kể khối lượng dữ liệu." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Writing individualized, contextually appropriate feedback in Vietnamese for every student after every class is time-consuming and repetitive.", vi: "Việc viết nhận xét cá nhân hóa, phù hợp ngữ cảnh bằng tiếng Việt cho mỗi học sinh sau mỗi buổi học rất tốn thời gian và lặp đi lặp lại." },
                        solution: { en: "Groq's inference API with prompt engineering that enforces Vietnamese tone, cultural context, and length constraints based on the student's performance data passed in at runtime.", vi: "Sử dụng API của Groq với prompt engineering để đảm bảo văn phong tiếng Việt, ngữ cảnh văn hóa và giới hạn độ dài dựa trên dữ liệu hiệu suất của học sinh được truyền vào lúc runtime." },
                        result: { en: "Context-aware, culturally appropriate Vietnamese feedback in < 300ms — replacing static templates with genuine intelligence.", vi: "Nhận xét tiếng Việt thông minh, phù hợp ngữ cảnh trong chưa đầy 300ms — thay thế các mẫu soạn sẵn bằng trí tuệ nhân tạo thực thụ." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Manual payment tracking creates reconciliation errors and time spent chasing parents for confirmation.", vi: "Theo dõi thanh toán thủ công gây ra lỗi đối soát và tốn thời gian yêu cầu phụ huynh xác nhận." },
                        solution: { en: "Dynamic VietQR generation embedded directly in PDF invoices, built to the NAPAS-247 standard. CRC-16 checksum validation ensures transaction data integrity end-to-end.", vi: "Tạo mã VietQR động nhúng trực tiếp vào hóa đơn PDF, xây dựng theo tiêu chuẩn NAPAS-247. Kiểm tra mã lỗi CRC-16 đảm bảo tính toàn vẹn của dữ liệu giao dịch." },
                        result: { en: "100% automated reconciliation flow. Error rate reduced from ~15% to near zero.", vi: "Quy trình đối soát tự động 100%. Tỷ lệ sai sót giảm từ ~15% xuống gần bằng 0." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Teaching online requires more than video — synchronized whiteboard, session recording, and graceful hardware fallback.", vi: "Dạy học trực tuyến cần nhiều hơn là chỉ video — cần bảng trắng đồng bộ, ghi âm buổi học và xử lý lỗi phần cứng linh hoạt." },
                        solution: { en: "Full-Mesh WebRTC for P2P media (server never touches the video stream). STOMP over WebSocket for state sync. Whiteboard throttled at 50ms cuts congestion 80%. Media Access Guard catches NotReadableError and surfaces human-readable guidance. Composite Stream Recording merges screen, webcam, and audio into a single .webm — entirely in-browser.", vi: "Sử dụng Full-Mesh WebRTC cho truyền tải media P2P (server không can thiệp vào luồng video). STOMP qua WebSocket để đồng bộ trạng thái. Bảng trắng được giới hạn ở mức 50ms giúp giảm 80% tắc nghẽn." },
                        result: { en: "< 200ms interactive latency. One-click recording. No crash on hardware conflicts.", vi: "Độ trễ tương tác dưới 200ms. Ghi hình một chạm. Không gặp lỗi khi xung đột phần cứng." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Calendar UIs feel slow when navigating months, and drag interactions drop frames on re-renders.", vi: "Giao diện lịch thường có cảm giác chậm khi chuyển tháng, và các tương tác kéo thả bị giật do render lại nhiều lần." },
                        solution: { en: "@dnd-kit with Optimistic Rollback. React.memo + useCallback eliminate 95% of unnecessary re-renders. Adjacent months prefetched automatically on load.", vi: "Sử dụng @dnd-kit với cơ chế Optimistic Rollback. React.memo và useCallback loại bỏ 95% các lần render lại không cần thiết. Các tháng liền kề được tải trước tự động khi tải trang." },
                        result: { en: "Month navigation instant (~0ms). Drag interactions at 60fps.", vi: "Chuyển tháng tức thì (~0ms). Tương tác kéo thả đạt 60fps mượt mà." },
                    ...
                    },
                    {
                        ...
                        problem: { en: "Digitizing exam questions from Word/PDF is tedious. Exam data loss from browser crashes or accidental navigation is unacceptable.", vi: "Số hóa câu hỏi thi từ Word/PDF rất tốn công. Việc mất dữ liệu bài làm do sập trình duyệt hoặc vô tình chuyển trang là điều không thể chấp nhận." },
                        solution: { en: "ExerciseParserService uses a Hybrid Regex Parsing Engine to bulk-ingest MCQ and Essay questions from plain text. Exam Player implements debounced auto-save (~500ms), timer sync, and auto-submit on expiry — fault-tolerant by design.", vi: "ExerciseParserService sử dụng công cụ Hybrid Regex để nhập hàng loạt câu hỏi trắc nghiệm và tự luận từ văn bản thuần túy. Trình làm bài thi thực hiện tự động lưu (auto-save) sau 500ms, đồng bộ bộ đếm thời gian và tự động nộp bài khi hết giờ." },
                        result: { en: "Question digitization time reduced by 90%. Zero data loss architecture.", vi: "Thời gian số hóa câu hỏi giảm 90%. Kiến trúc đảm bảo không bao giờ mất dữ liệu." },
                ...
                decisions: [
                    {
                        ...
                        reason: { en: "Solo project — microservices would mean 10× the DevOps overhead with no team to justify it. Clear module boundaries make extraction straightforward if scale ever demands it.", vi: "Dự án độc lập — microservices sẽ tăng gánh nặng DevOps gấp 10 lần mà không có đội ngũ để vận hành. Ranh giới mô-đun rõ ràng giúp việc tách service dễ dàng nếu sau này cần mở rộng." },
                    },
                    {
                        ...
                        reason: { en: "Notifications flow in one direction. SSE is HTTP-native, proxy-transparent, and requires no handshake upgrade. WebSocket is reserved for the live room where true bidirectionality is actually needed.", vi: "Thông báo chỉ luân chuyển một chiều từ server. SSE chạy trên nền HTTP, đi xuyên qua proxy và không cần nâng cấp kết nối (handshake). WebSocket chỉ dành cho phòng học trực tuyến nơi cần tương tác hai chiều thực thụ." },
                    },
                    {
                        ...
                        reason: { en: "All async state is server state. React Query's stale-while-revalidate, optimistic updates, and cache invalidation handle this natively. Redux would be ceremonial overhead with no real benefit.", vi: "Mọi trạng thái không đồng bộ đều là dữ liệu từ server. React Query xử lý tốt các cơ chế caching, cập nhật tức thì (optimistic updates) một cách tự nhiên. Dùng Redux sẽ rườm rà mà không mang lại lợi ích thực tế cho dự án này." },
                    },
                ],
            },
            {
    ...
    // ─────────────────────────────────────────────
    {
        ...
        accentColor: "#00e5a0",
        stack: [
            "Spring Boot (Java 21)", "Next.js", "MySQL (per service)",
        ...
        overview: {
            ...
            vi: "Dự án nhóm 6 người nhằm xây dựng nền tảng điều phối thời gian thực cho các hoạt động cứu hộ lũ lụt tại Việt Nam. Với vai trò Tech Lead, trách nhiệm của tôi là thiết kế kiến trúc, lựa chọn công nghệ, thiết lập toàn bộ hạ tầng, quy trình CI/CD, khung dự án (frontend + backend) và quản lý sprint qua Jira.",
        },
        sections: [
            {
                type: "problem",
                ...
                features: [
                    {
                        ...
                        problem: { en: "Citizens have no way to submit structured rescue requests or track status. Coordinators receive requests through informal channels (phone, Zalo) with no geographic data.", vi: "Người dân không có cách nào gửi yêu cầu cứu hộ có cấu trúc hoặc theo dõi trạng thái. Điều phối viên nhận yêu cầu qua các kênh không chính thống (điện thoại, Zalo) và thiếu dữ liệu vị trí." },
                        solution: { en: "", vi: "" },
                        result: { en: "", vi: "" },
                    },
                    {
                        title: { en: "No live team tracking", vi: "Không theo dõi được vị trí đội cứu hộ" },
                        problem: { en: "Coordinators can't see where rescue teams are in real-time. Assignment is done by phone call without proximity awareness.", vi: "Điều phối viên không thể thấy vị trí các đội cứu hộ trong thời gian thực. Việc phân công thực hiện qua điện thoại mà không biết rõ khoảng cách thực tế." },
                        solution: { en: "", vi: "" },
                        result: { en: "", vi: "" },
                    },
                    {
            ...
            },
            {
                ...
                features: [
                    {
                        title: { en: "Architecture Diagram", vi: "Sơ đồ Kiến trúc" },
                        solution: { en: "", vi: "" },
                        result: { en: "", vi: "" },
                        media: "/case-studies/frc-architecture.png",
                    },
                ],
            },
            {
                type: "result",
                title: { en: "Sprint 1 Progress — What's Built", vi: "Tiến độ Sprint 1 — Đã Xây dựng" },
                content: {
                    en: "Sprint 1 (4 Mar – 11 Mar): 6 of 10 tasks DONE, 1 IN PROGRESS. Core infrastructure and service skeletons are live.",
                    vi: "Sprint 1 (4/3 – 11/3): 6/10 tasks DONE, 1 đang thực hiện. Hạ tầng cốt lõi và skeleton các service đã hoạt động.",
                },
                metrics: [
                    { value: "6/10", label: { en: "Sprint 1 tasks completed", vi: "Tasks Sprint 1 hoàn thành" } },
                    { value: "✓", label: { en: "Auth + API Gateway live", vi: "Auth + API Gateway hoạt động" } },
                    { value: "✓", label: { en: "RescueRequest + Dispatch impl", vi: "RescueRequest + Dispatch đã implement" } },
                    { value: "✓", label: { en: "FE + BE module skeleton", vi: "Skeleton FE + BE hoàn chỉnh" } },
                    { value: "▶", label: { en: "SSE Notification (in progress)", vi: "SSE Notification (đang làm)" } },
                ],
            },
            {
                type: "decisions",
                ...
                decisions: [
                    {
                        ...
                        reason: { en: "A 6-person team starting with 6 independent microservices means 6× the DevOps complexity before any business logic is validated. We validate domain boundaries in Sprint 1 as a modular monolith, then extract services once the seams are clear.", vi: "Một đội ngũ 6 người bắt đầu với 6 microservices độc lập sẽ gặp khó khăn về DevOps gấp 6 lần trước khi logic nghiệp vụ được xác thực. Chúng tôi xác định ranh giới miền trong Sprint 1 dưới dạng modular monolith, sau đó tách service khi cấu trúc đã ổn định." },
                    },
                    {
                        ...
                        reason: { en: "RabbitMQ gives exactly-once delivery for rescue task assignments — critical when one team must be assigned to exactly one request. Kafka gives a replayable audit log of all GPS positions and status changes — essential for post-incident analysis and reporting.", vi: "RabbitMQ đảm bảo giao nhận chính xác một lần cho các nhiệm vụ cứu hộ — quan trọng khi một đội chỉ được phân công đúng một yêu cầu. Kafka cung cấp nhật ký có thể phát lại của toàn bộ vị trí GPS và thay đổi trạng thái — thiết yếu cho phân tích sau sự cố." },
                    },
                    {
            ...
            },
            {
                ...
                features: [
                    { title: { en: "user-service :8081", vi: "user-service :8081" }, problem: { en: "", vi: "" }, solution: { en: "Auth, JWT, role management.", vi: "Xác thực, JWT, quản lý vai trò." }, result: { en: "5 actor roles: Citizen, Rescue Team, Coordinator, Manager, Admin.", vi: "5 loại người dùng: Người dân, Đội cứu hộ, Điều phối viên, Quản lý, Admin." } },
                    { title: { en: "rescue-request-service :8082", vi: "rescue-request-service :8082" }, problem: { en: "", vi: "" }, solution: { en: "GPS-tagged request submission, status FSM.", vi: "Gửi yêu cầu kèm vị trí GPS, máy trạng thái (FSM)." }, result: { en: "PENDING → VERIFIED → DISPATCHED → COMPLETED with citizen SSE notifications.", vi: "PENDING → VERIFIED → DISPATCHED → COMPLETED kèm thông báo SSE cho người dân." } },
                    { title: { en: "dispatch-service :8083", vi: "dispatch-service :8083" }, problem: { en: "", vi: "" }, solution: { en: "Map-based team assignment, proximity calculation.", vi: "Phân công đội cứu hộ dựa trên bản đồ, tính toán khoảng cách." }, result: { en: "10s GPS auto-reporting, live coordinator tracking view.", vi: "Tự động báo cáo GPS mỗi 10 giây, chế độ xem theo dõi cho điều phối viên." } },
                    { title: { en: "resource-service :8084", vi: "resource-service :8084" }, problem: { en: "", vi: "" }, solution: { en: "Relief supply warehouse, allocation tracking.", vi: "Quản lý kho hàng cứu trợ, theo dõi phân bổ." }, result: { en: "Low-stock alerts, distribution records.", vi: "Cảnh báo hết hàng, nhật ký phân phối." } },
                    { title: { en: "notification-service :8085", vi: "notification-service :8085" }, problem: { en: "", vi: "" }, solution: { en: "SSE emitter pool consuming RabbitMQ events.", vi: "Pool SSE emitter tiêu thụ sự kiện từ RabbitMQ." }, result: { en: "Real-time status updates to all actor types.", vi: "Cập nhật trạng thái thời gian thực cho mọi vai trò." } },
                    { title: { en: "reporting-service :8086", vi: "reporting-service :8086" }, problem: { en: "", vi: "" }, solution: { en: "KPI aggregation, Kafka consumer for audit streams.", vi: "Tổng hợp KPI, Kafka consumer cho luồng audit." }, result: { en: "Operational analytics for Manager/Admin roles.", vi: "Phân tích vận hành cho vai trò Quản lý/Admin." } },
                ],
            },
            {
                type: "code",
